...
def main():
    ...
    result_dih, func_result_dih, data_table_dih = dih(
        task1, eps)
    print(
        f"Dihotomy: Xmin = {result_dih}, Fmin = {func_result_dih}, Accuracy = {eps}")
    ...
    result_gold, func_result_gold, data_table_golden_ratio = golden(
        task1, eps)
    print(
        f"Golden Ratio: Xmin = {result_gold}, Fmin = {func_result_gold}, Accuracy = {eps}")

    # -------------------------------------------------------------------------

    result_search_a, result_search_b, data_table_search = search(eps, 0.)
    print(
        f"Search Interval: Xmin from [{result_search_a};{result_search_b}] Accuracy = {eps}")
